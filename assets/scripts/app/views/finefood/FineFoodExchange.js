...
var i = require("../../views/item/ItemSlotUI"),
    ...
    u = (function (t) {
        ...
        e.prototype.onLoad = function () {
            //界面控制
            facade.subscribe(l.fineFoodProxy.ON_FINE_FOOD_EX_CLOTHE_CLICK, this.onClotheSelect, this);
            facade.subscribe(l.fineFoodProxy.FINE_FOOD_SPECIAL_EXCHANGE_BACK, this.updateShow, this);

            this.shopData.hid = l.fineFoodProxy.data && l.fineFoodProxy.data.info ? l.fineFoodProxy.data.info.type : 1;
            this.shopData.stime = l.fineFoodProxy.data && l.fineFoodProxy.data.info ? l.fineFoodProxy.data.info.showTime : 0;

            //商城标题
            this.lblShopName.string = i18n.t("FINE_FOOD_SHOP_EXCHANGE_TITLE") + i18n.t("FINE_FOOD_SHOP_PROCESS_TITLE"); //i18n.t(t);
        ...
        e.prototype.updateShow = function () {
            //刷新列表
            this.shopData = [];
            if (l.fineFoodProxy.specialExchange) {
                var tempList = [];
                for (var i = 0; i < l.fineFoodProxy.specialExchange.length; ++i) {
                    var item = l.fineFoodProxy.specialExchange[i];
                    if (item.tag == 1) {
                        item.index = i;
                        item.select = false;
                        tempList.push(item);
        ...
        e.prototype.onClotheSelect = function (index) {
            var data = this.shopData.find(function (it) {
        ...
        e.prototype.onCurItem = function (data) {
            ...
            for (var idx = 0; idx < this.fishList.length; idx++) {
                ...
                if (show) {
                    var no = this.fishList[idx].getComponent("FineFoodExchangeFishItem");
                    if (no) {
        ...
        e.prototype.onClickExchange = function () {
            if (this.firstItem.data) {
                l.fineFoodProxy.sendSpecialExchange(this.firstItem.data.index, 1);
            }
        };
