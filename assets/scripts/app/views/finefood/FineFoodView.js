...
var i = require("../../utils/Utils"),
    ...
    buildItem = require("./FineFoodPoint"),
    c = cc._decorator,
    _ = c.ccclass,
    d = c.property,
    u = (function (t) {
        ...
        e.prototype.onLoad = function () {
            //更新数据
            facade.subscribe(a.fineFoodProxy.FINE_FOOD_DATA_UPDATE, this.onDataUpdate, this);
            //更新体力
            facade.subscribe(a.fineFoodProxy.FINE_FOOD_ACT_UPDATE, this.onActUpdate, this);

            //更新物品数量
            facade.subscribe(a.bagProxy.UPDATE_BAG_ITEM, this.onItemUpdate, this);
            //点击地点
            facade.subscribe(a.fineFoodProxy.ON_FINE_FOOD_CLICK_POINT, this.onPlayPoint, this);
            //开始移动
            facade.subscribe(a.fineFoodProxy.ON_FINE_FOOD_ACT_BACK, this.onPlayBack, this);

            //钓鱼以后返回的时候把马车开回原点
            facade.subscribe(a.fineFoodProxy.FINE_FOOD_FISHING_BACK, this.goBack, this);

            //请求数据
            a.fineFoodProxy.sendOpenFineFood();
        };
        ...
        e.prototype.init = function () {
            ...
            for (var t = 0; t < pointList.length; t++) {
                ...
                //加载服务器发过来的路点以及原点
                var item = a.fineFoodProxy.cities[id];
                if (0 == id || item) {
        ...
        e.prototype.onDataUpdate = function () {
            ...
            //倒数计时
            if (a.fineFoodProxy.data) {
                var t = this;
                l.uiUtils.countDown(
                    a.fineFoodProxy.data.info.eTime,
                    this.lblTime,
                    function () {
        ...
        e.prototype.onStaminaCounDown = function () {
            this.lblStaminaCountDown.node.active = a.fineFoodProxy.act.stamina < 1;
            this.lblStamina.node.active = a.fineFoodProxy.act.stamina >= 1;

            if (a.fineFoodProxy.act.stamina < 1) {
                var cur = utils.timeUtil.second + (1 - a.fineFoodProxy.act.stamina) * a.fineFoodProxy.data.settings.stamina.recover_per_second;
        ...
        e.prototype.updateStamina = function () {
            if (a.fineFoodProxy.data && a.fineFoodProxy.act) {
                var curStamina = a.fineFoodProxy.act.stamina;
                ...
                this.lblStamina.string =
                    parseInt(curStamina).toString() + " / " + a.fineFoodProxy.data.settings.stamina.max.toString();

                //刚进地图如果有点，就直接放过去
                if (!this.initPt) {
                    //初始化马车位置
                    if (a.fineFoodProxy.act.mid_place >= 0) {
                        this._oldId = a.fineFoodProxy.act.mid_place;
                    ...
                    if (a.fineFoodProxy.act.type <= 0) {
                        //如果type = 0 说明没有移动
                        return;
                    }

                    //否则移动过去
                    if (a.fineFoodProxy.act.mid_place) {
                        this.onPlayBack(a.fineFoodProxy.act.mid_place);
                    }
                }
            }
        };
        ...
        e.prototype.onPlayPoint = function (pt) {
            if (this.onMove) {
                utils.alertUtil.alert18n("FINE_FOOD_MAIN_WAIT_BACK");
                return;
            }
            ...
            //如果当前点没有动，就直接进去
            if (a.fineFoodProxy.act.mid_place == pt.id) {
            ...
            //打开info
            i.utils.openPrefabView("finefood/FineFoodBuildInfo", null, pt);
        };
        ...
        e.prototype.onArrived = function (ptTab) {
            ...
            var sevTab = a.fineFoodProxy.cities[ptTab.id];
            if (sevTab) {
                //如果place 大于0说明需要钓鱼
                if (a.fineFoodProxy.act.place > 0) {
                    //进城
                    ...
                    a.fineFoodProxy.act.mid_place = 0;
                    a.fineFoodProxy.act.place = 0;
                    this.goBack(1);
                }
            }
        ...
        e.prototype.onClickPlay = function (t, e) {
            ...
            //如果此地尚未垂钓，就出提示
            if (a.fineFoodProxy.act.place == a.fineFoodProxy.act.mid_place && a.fineFoodProxy.act.place > 0) {
                var self = this;
                i.utils.showConfirm(i18n.t("FINE_FOOD_MAIN_MOVE_TIP"), function () {
                    self.doPlay();
                });
        ...
        e.prototype.doPlay = function () {
            //如果没有在移动状态
            if (a.fineFoodProxy.act) {
                if (parseInt(a.fineFoodProxy.act.stamina) < a.fineFoodProxy.data.settings.walk.random_cost) {
                    //体力不够
                    if (!i.utils.isOpenView("FineFoodStaminaBuy")) {
                        i.utils.openPrefabView("finefood/FineFoodStaminaBuy", null, null);
                    }
                    /*
                    i.utils.showConfirm(i18n.t("FINE_FOOD_MAIN_STAMINA_NOT_ENOUGH", {num: a.fineFoodProxy.data.settings.stamina.price})
                    , function() {
                        a.fineFoodProxy.sendBuyStamina();
                    });
                    */
                    return;
                }
            ...
            //体力够用
            this.startMove(false);
            a.fineFoodProxy.sendMove(0); //发消息随机移动
        };

        //打开钓鱼界面
        e.prototype.openFishingView = function () {
            if (!i.utils.isOpenView("FineFoodFishingView")) {
                i.utils.openPrefabView("finefood/FineFoodFishingView", null, a.fineFoodProxy.act.place);
            }
        };
        ...
        e.prototype.onClickBuild = function (t, e) {
            ...
            i.utils.openPrefabView("finefood/FineFoodBuildInfo", null, e);
        };

        //更新物品数量
        e.prototype.onItemUpdate = function () {
            this.lblRodsCount.string = a.bagProxy.getItemCount(a.fineFoodProxy.data.settings.fishing.rod) + "";
        };
            ...
            switch (e) {
                case "1":
                    i.utils.openPrefabView(
                        "finefood/FineFoodShopView",
                        null,
                        null //a.fineFoodProxy.data.exchange
                    );
                    break;
                ...
                case "2":
                    i.utils.openPrefabView(
                        ...
                        null, {
                            type: a.limitActivityProxy.FINE_FOOD_TYPE
                        }
                    );
                    break;

                case "3":
                    i.utils.openPrefabView("finefood/FineFoodRwd");
                    break;

                case "4":
                    i.utils.openPrefabView("ActivitySpecialBuy", null, {
                        data: a.fineFoodProxy.shop[0],
                        activityId: a.fineFoodProxy.data.info.type
                    });
            }
        };
