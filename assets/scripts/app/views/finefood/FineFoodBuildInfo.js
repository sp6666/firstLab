...
var i = require("../../component/List"),
    ...
    c = (function (t) {
        ...
        e.prototype.onLoad = function () {
            facade.subscribe(l.fineFoodProxy.FINE_FOOD_ACT_UPDATE, this.onActUpdate, this);

            this.data = this.node.openParam;
            var curId = this.data.id;
            this.sevData = l.fineFoodProxy.cities[curId];
        ...
        e.prototype.updateData = function () {
            ...
            if (this.sevData.fishes.cheap || this.sevData.fishes.special) {
                this.lblReward.string = i18n.t("FINE_FOOD_MAIN_TAKE"); //显示将会获得

                if (this.sevData.fishes.cheap) //一般鱼
                {
                    this.cashCity = l.fineFoodProxy.data.settings.walk.teleport_normal_fishing_city_cost; //入城费
                ...
                {
                    this.cashCity = l.fineFoodProxy.data.settings.walk.teleport_special_fishing_city_cost; //入城费
                ...
                this.lblReward.string = i18n.t("FINE_FOOD_MAIN_MABY_TAKE"); //显示可能获得
                this.cashCity = l.fineFoodProxy.data.settings.walk.teleport_normal_city_cost; //入城费

                for (var idx = 0; idx < l.fineFoodProxy.data.city_random_rwds.length; idx++) {
                    var item = {};
                    item.id = l.fineFoodProxy.data.city_random_rwds[idx].id;
                    item.count = l.fineFoodProxy.data.city_random_rwds[idx].count;
                    rwds.push(item);
                }
            }
            ...
            //按钮 活动未结束就打开
            this.btnLook.interactable = l.fineFoodProxy.data.info.eTime > n.timeUtil.getCurSceond();
        };

        //入城扣钱
        e.prototype.onClickLook = function () {
            //如果此地尚未垂钓，就出提示
            if (l.fineFoodProxy.act.place == l.fineFoodProxy.act.mid_place && l.fineFoodProxy.act.place > 0) {
                var self = this;
                n.utils.showConfirm(i18n.t("FINE_FOOD_MAIN_MOVE_TIP"), function () {
                    self.doPlay();
                });
        ...
        e.prototype.doPlay = function () {
            ...
            if (l.fineFoodProxy.act) {
                if (l.fineFoodProxy.act.stamina < l.fineFoodProxy.data.settings.walk.random_cost) {
                    //体力不够
                    if (!n.utils.isOpenView("FineFoodStaminaBuy")) {
                        n.utils.openPrefabView("finefood/FineFoodStaminaBuy", null, null);
                    }
                    return;
                }
            }
            ...
            //体力元宝都够了就移动
            l.fineFoodProxy.sendMove(this.data.id);
        };

        e.prototype.onActUpdate = function () {
            if (l.fineFoodProxy.act.type <= 0) {
                //如果没有place说明仅仅是买了体力
                return;
            }

            //目前能取到就关闭,传id过去
            facade.send(l.fineFoodProxy.ON_FINE_FOOD_ACT_BACK, this.data.id);
            this.onClickClose();
        };
