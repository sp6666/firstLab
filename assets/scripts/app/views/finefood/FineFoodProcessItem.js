...
cc.Class({
    ...
    onClickGift: function () {
        if (this.silderCount.curValue <= 0) {
            //提示道具不足
            u.alertUtil.alert(i18n.t("FINE_FOOD_SHOP_EXCHANGE_NO_NUM", {
        ...
        var self = this;
        u.utils.showConfirm(
            i18n.t("FINE_FOOD_SHOP_PROCESS_TIP", {
                num: self.silderCount.curValue,
                name: self.lblName.string,
                ypnum: self._data.target[0].count * self.silderCount.curValue
            }),
            function () {
                i.fineFoodProxy.sendSpecialExchange(self._data.index, self.silderCount.curValue);
            }
        );
    },
});