...
cc.Class({
    ...
    onLoad () {
        facade.subscribe(Initializer.zhouNianProxy.SHOP_UPDATE2,this.onDataUpdate,this);
        this.shopData = this.node.openParam;
        this.suitCtrs = [];
        for (var i = 0; i < this.suitNode.length; i++) {
            this.suitCtrs.push(this.suitNode[i].getComponent("ZhouNianDHZItem"));
        }
        this.updateShow();
    },
    ...
    onClickExchange : function(evt, customData) {
        ...
        Initializer.zhouNianProxy.sendExchange(type);
    },
});
